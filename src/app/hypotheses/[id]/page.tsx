...
import { HypothesisEditor } from "@/components/hypothesis-editor";
...
export default function HypothesisPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="standard-page">
    ...
    <HypothesisEditor id={id} item={item} onSaved={load}/>
    ...
    {item.revisions?.length>0&&<section className="revision-ledger"><p className="eyebrow">Revision history</p>{item.revisions.map((revision:Record<string,any>)=><article key={revision.id}><b>Revision {revision.revision_number}</b><span>{revision.reason}</span><small>{revision.actor} · {new Date(revision.created_at).toLocaleString()}</small></article>)}</section>}
