...
export default function SacredTextsPage(){
  ...
  return <main className="sacred-workbench">
    ...
    {result&&<section className="sacred-results">
      ...
      <div className="sacred-tradition-columns">{grouped.map(([tradition,items])=><section key={tradition}><header><span>{tradition}</span><b>{items.length} matches</b></header>{items.map(item=><article key={item.passage_id}>
